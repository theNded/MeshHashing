...
void PlaneFittingKernel(
  ...
  if (threadIdx.x == 0) {
    ...
    for (int iter = 0; iter < 10; ++iter) {
      ...
      for (int j = 0; j < vertex_count; ++j) {
        ...
        if (dist < 0.008) inliner++;
      }

      ratio = float(inliner) / vertex_count;
      if (ratio > 0.95) break;
    }
  }
  __syncthreads();
  ...
  if (blocks[entry.ptr].voxels[local_idx].weight == 0) return;
  float osdf = blocks[entry.ptr].voxels[local_idx].sdf;
  float nsdf = dot(n, world_pos) + d;

  if (fabs(nsdf) < 0.008 && fabs(nsdf - osdf) < 0.008f) {
