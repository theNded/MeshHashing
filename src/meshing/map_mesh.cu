...
void Map::SaveMesh(std::string path) {
  ...
  checkCudaErrors(cudaMemcpy(vertices, compact_mesh_.gpu_memory().vertices,
                             sizeof(float3) * compact_vertex_count,
                             cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(normals, compact_mesh_.gpu_memory().normals,
                             sizeof(float3) * compact_vertex_count,
                             cudaMemcpyDeviceToHost));
  checkCudaErrors(cudaMemcpy(triangles, compact_mesh_.gpu_memory().triangles,
