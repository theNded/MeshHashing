...
Sensor::Sensor(SensorParams &sensor_params) {
  ...
  checkCudaErrors(cudaMalloc(&gpu_memory_.depth_image, sizeof(float) * image_size));
  checkCudaErrors(cudaMalloc(&gpu_memory_.color_image, sizeof(float4) * image_size));

  gpu_memory_.depth_channel_desc
      = cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat);
  checkCudaErrors(cudaMallocArray(&gpu_memory_.depth_array,
                                  &gpu_memory_.depth_channel_desc,
                                  sensor_params_.width, sensor_params_.height));
  gpu_memory_.color_channel_desc
      = cudaCreateChannelDesc(32, 32, 32, 32, cudaChannelFormatKindFloat);
  checkCudaErrors(cudaMallocArray(&gpu_memory_.color_array,
                                  &gpu_memory_.color_channel_desc,
                                  sensor_params_.width, sensor_params_.height));

  gpu_memory_.depth_texture = 0;
  gpu_memory_.color_texture = 0;
...
Sensor::~Sensor() {
  ...
  checkCudaErrors(cudaFreeArray(gpu_memory_.depth_array));
  checkCudaErrors(cudaFreeArray(gpu_memory_.color_array));
}
...
void Sensor::BindGPUTexture() {
  ...
  depth_resource.res.array.array = gpu_memory_.depth_array;
  ...
  if (gpu_memory_.depth_texture != 0)
    checkCudaErrors(cudaDestroyTextureObject(gpu_memory_.depth_texture));
  checkCudaErrors(cudaCreateTextureObject(&gpu_memory_.depth_texture, &depth_resource, &depth_tex_desc, NULL));

  cudaResourceDesc color_resource;
  memset(&color_resource, 0, sizeof(color_resource));
  color_resource.resType = cudaResourceTypeArray;
  color_resource.res.array.array = gpu_memory_.color_array;
  ...
  if (gpu_memory_.color_texture != 0)
    checkCudaErrors(cudaDestroyTextureObject(gpu_memory_.color_texture));
  checkCudaErrors(cudaCreateTextureObject(&gpu_memory_.color_texture, &color_resource, &color_tex_desc, NULL));
}
...
int Sensor::Process(cv::Mat &depth, cv::Mat &color) {
  ...
  checkCudaErrors(cudaMemcpyToArray(gpu_memory_.depth_array, 0, 0, gpu_memory_.depth_image,
                                    sizeof(float)*sensor_params_.height*sensor_params_.width,
                                    cudaMemcpyDeviceToDevice));
  checkCudaErrors(cudaMemcpyToArray(gpu_memory_.color_array, 0, 0, gpu_memory_.color_image,
...
void Sensor::DepthCPUtoGPU(cv::Mat& depth) {
  ...
  DepthCPUtoGPUKernel<<<grid_size, block_size>>>(
          gpu_memory_.depth_image, depth_imagebuffer_,
          width, height,
...
void Sensor::ColorCPUtoGPU(cv::Mat &color) {
  ...
  ColorCPUtoGPUKernel<<<grid_size, block_size>>>(
          gpu_memory_.color_image, color_imagebuffer_, width, height);
}
...
float4* Sensor::ColorizeDepthImage() const {
  ...
  DepthToRGBKernel<<<grid_size, block_size>>>(
          colored_depth_image_, gpu_memory_.depth_image,
          sensor_params_.width, sensor_params_.height,
          sensor_params_.min_depth_range,
          sensor_params_.max_depth_range);
