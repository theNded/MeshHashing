/// Extracted from voxel hashing
#ifndef CUDA_MATRIX_H
#define CUDA_MATRIX_H

#pragma once

/// Modified according to voxel hashing
...
#define logged_assert(condition) if (!(condition)) { printf("ASSERT: %s %s\n", #condition, __FILE__); }
...
public:
  ...
  {
    logged_assert(xStart+NOther <= N && yStart+MOther <= M);
