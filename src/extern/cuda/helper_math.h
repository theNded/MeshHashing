...
/// Added by wei
inline __host__ __device__ int sign(float s) {
  return s < 0 ? -1 : 1;
}

inline __host__ __device__ int3 sign(float3 s) {
  return make_int3(sign(s.x), sign(s.y), sign(s.z));
}

inline __host__ __device__ int2 sign(float2 s) {
