...
struct HashData {
  ...
  void allocate(const HashParams &params, bool dataOnGPU = true) {
    ...
    if (m_bIsOnGPU) {
      ...
      checkCudaErrors(cudaMalloc(&d_SDFBlocks,
  ...
  HashData copyToCPU() const {
    ...
    checkCudaErrors(cudaMemcpy(hashData.d_hashDecision, d_hashDecision,
                               sizeof(int) * params.m_hashNumBuckets * params.m_hashBucketSize,
                               cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(hashData.d_hashDecisionPrefix, d_hashDecisionPrefix,
                               sizeof(int) * params.m_hashNumBuckets * params.m_hashBucketSize,
                               cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(hashData.d_hashCompactified, d_hashCompactified,
                               sizeof(HashEntry) * params.m_hashNumBuckets * params.m_hashBucketSize,
                               cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(hashData.d_hashCompactifiedCounter, d_hashCompactifiedCounter, sizeof(unsigned int),
                               cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(hashData.d_SDFBlocks, d_SDFBlocks,
                               sizeof(Voxel) * params.m_numSDFBlocks * params.m_SDFBlockSize * params.m_SDFBlockSize *
                               params.m_SDFBlockSize, cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(hashData.d_hashBucketMutex, d_hashBucketMutex, sizeof(int) * params.m_hashNumBuckets,
                               cudaMemcpyDeviceToHost));
  ...
  uint *d_heap;               /// heap that manages free memory (for SDFBlocks)
  ...
  HashEntry *d_hash;          /// hash entries that stores pointers to sdf blocks

  // assume this is only used for streaming
